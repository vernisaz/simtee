...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if cli.get_opt("v").unwrap().is_some() {
        ...
        return Ok(());
    } else if cli.get_opt("h").unwrap().is_some() {
        println!(
    ...
    let overwrite = cli.get_opt("w").unwrap().is_some();
    let mut out: Box<dyn Write> = if let Some(OptVal::Str(name)) = cli.get_opt("o").unwrap()
        && cli.get_opt("r").unwrap().is_none()
    {
        Box::new(
            File::options()
                .truncate(overwrite)
